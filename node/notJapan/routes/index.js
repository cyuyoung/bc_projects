...
/* GET home page. */
//요청이 들어오면 해석하는데, 만약 get방식이고 주소가 /이면 아래 함수가 호출된다. => 라우팅
router.get('/', function (req, res, next) {
  ...
  //views/index.ejs 파일을 읽어서 전달한 데이터를 사용하여 완성된 html을 응답한다. => 랜더링한다 
  //랜더링 대상  ejs파일은 무조건 views 밑에서 찾는다
  res.render('index', { title: '점심시간은 11시 50분', name:req.session.name });//{} 객체 리터럴 일회성
});
...
router.get('/search', (req, res, next) => {
  ...
  db.selectNoSellData((err, rows) => {
    ...
    //4. 만약 카테고리가 금융이면 박스의 배경색을 bg-yellow로 설정 
  })
});
//~/search, post
router.get('/search2', (req, res, next) => {
  ...
  //3. 디비 쿼리를 수행, 검색어 넣어서 수행하는 쿼리 결과
  //함수명 : selectItemByKeyword()
...
//동적파라미터는 url에 데이터를 넣어서 전송하는 방식 -> url은 무한대로 확장이 가능하다
//get 방식, post방식 등 method 방식과 혼용해서 사용 가능
//~/url/:변수명
